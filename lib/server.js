...
class TranslationServer {
    constructor(config) {
        ...
        this.config = {
            ...
            translation: {
                apiUrl:
                    config.translation?.apiUrl || process.env.TRANSLATE_API_URL,
                apiKey:
                    config.translation?.apiKey ||
                    process.env.TRANSLATE_API_AUTHORIZATION,
                prompt:
                    config.translation?.prompt || process.env.TRANSLATE_PROMPT,
    ...
    setupMiddleware() {
        ...
        this.app.get("/translation-manager.html", (req, res) => {
            const templatePath = path.join(
                __dirname,
                "../templates/translation-manager.html"
            )
    ...
    setupRoutes() {
        ...
        this.app.get("/api/translations", (req, res) => {
            try {
                ...
                res.status(500).json({
                    error: "Failed to get translations: " + error.message,
                })
            }
        })
        ...
        this.app.get("/api/translations/:id", (req, res) => {
            try {
                ...
                if (!translation) {
                    return res
                        .status(404)
                        .json({ error: "Translation not found" })
                }

                res.json(translation)
            } catch (error) {
                res.status(500).json({
                    error: "Failed to get translation: " + error.message,
                })
            }
        })
        ...
        this.app.post("/api/translations", (req, res) => {
            try {
                ...
                const existingData = this.fileManager.readTranslationFile(
                    "zh",
                    file
                )
                if (existingData[key] !== undefined) {
                    return res
                        .status(409)
                        .json({ error: "Translation key already exists" })
                }

                if (
                    this.fileManager.updateTranslation(key, file, translations)
                ) {
                    res.status(201).json({
                        key,
                        file,
                ...
                res.status(500).json({
                    error: "Failed to add translation: " + error.message,
                })
            }
        })
        ...
        this.app.put("/api/translations/:id", (req, res) => {
            try {
                ...
                if (key !== translation.key || file !== translation.file) {
                    this.fileManager.deleteTranslation(
                        translation.key,
                        translation.file
                    )
                }
        ...
        this.app.post("/api/batch-delete", (req, res) => {
            try {
                const { ids } = req.body
                ...
                for (const id of ids) {
                    ...
                    if (translation) {
                        if (
                            ...
                            errors.push(`Failed to delete ID ${id}`)
                        }
                    } else {
                        errors.push(`Translation ID ${id} not found`)
                    }
                }
            ...
            try {
                ...
                if (!sourceText) {
                    return res
                        .status(400)
                        .json({
                            error: `Source language ${sourceLanguage} translation not found`,
                        })
                }
        ...
        this.app.post("/api/translate-only", async (req, res) => {
            try {
                ...
                if (!text) {
                    return res
                        .status(400)
                        .json({ error: "Please provide text to translate" })
                }
        ...
        this.app.post("/api/batch-translate", async (req, res) => {
            try {
                ...
                if (!ids || !Array.isArray(ids) || ids.length === 0) {
                    return res
                        .status(400)
                        .json({ error: "Please provide IDs to translate" })
                }
                ...
                for (const id of ids) {
                    ...
                    if (!sourceText) {
                        errors.push(
                            `Source language ${sourceLanguage} for ID ${id} not found`
                        )
                        continue
                    }
        ...
        this.app.post("/api/create-file", async (req, res) => {
            try {
                ...
                if (!fileName) {
                    return res
                        .status(400)
                        .json({ error: "Please provide file name" })
                }

                const { createdCount, errors } =
        ...
        this.app.get("/api/export-data", (req, res) => {
            try {
                ...
                if (!fs.existsSync(localesPath)) {
                    return res
                        .status(404)
                        .json({ error: "Locales folder not found" })
                }
        ...
        return new Promise((resolve) => {
            ...
            this.server = this.app.listen(
                ...
                () => {
                    ...
                    console.log(
                        `   API Server: http://${this.config.host}:${this.config.port}`
                    )
                    console.log(
                        `   Web UI:     http://${this.config.host}:${this.config.port}/translation-manager.html`
                    )
                    ...
                    console.log(
                        "   POST   /api/auto-translate/:id - Auto-translate"
                    )
                    console.log(
                        "   POST   /api/translate-only   - Translate only (no save)"
                    )
                    console.log(
                        "   POST   /api/batch-translate  - Batch translate"
                    )
                    console.log(
                        "   POST   /api/refresh-cache    - Refresh cache"
                    )
